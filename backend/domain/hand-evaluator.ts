/**
 * backend/domain/hand-evaluator.ts - Poker Hand Evaluation Engine
 *
...
import { HAND_RANKS, HAND_NAMES, RANK_VALUES } from '../config/constants';
...
export const HandEvaluator = {
   ...
   * @returns HandResult with rank, name, cards, kickers
   */
   ...
   */
  compare(handA: HandResult, handB: HandResult): number {
   ...
   */
  beats(handA: HandResult, handB: HandResult): boolean {
   ...
   * @param holeCards - 2 cards
   * @returns 0-100 strength score
   */
  holeCardStrength(holeCards: Card[]): number {
  ...
  _checkStraightFlush(cards: Card[]): HandResult | null {
  ...
  _checkFourOfAKind(cards: Card[]): HandResult | null {
  ...
  _checkFullHouse(cards: Card[]): HandResult | null {
    const byValue = this._groupByValue(cards);
    ...
    if (triples.length >= 1 && (triples.length >= 2 || pairs.length >= 2)) {
      ...
      let bestPairValue = 0;
      let bestPairCards: Card[] | null = null;
  ...
  /** Check for Flush */
  _checkFlush(cards: Card[]): HandResult | null {
    const bySuit = this._groupBySuit(cards);
  ...
  _checkStraight(cards: Card[]): HandResult | null {
  ...
  _checkThreeOfAKind(cards: Card[]): HandResult | null {
    const byValue = this._groupByValue(cards);
    const triples = Object.entries(byValue).filter(([_, cs]) => cs.length >= 3);
  ...
  /** Check for Two Pair */
  _checkTwoPair(cards: Card[]): HandResult | null {
    const byValue = this._groupByValue(cards);
    const pairs = Object.entries(byValue).filter(([_, cs]) => cs.length >= 2);
  ...
  /** Check for One Pair */
  _checkOnePair(cards: Card[]): HandResult | null {
    const byValue = this._groupByValue(cards);
    const pairs = Object.entries(byValue).filter(([_, cs]) => cs.length >= 2);
  ...
  _checkHighCard(cards: Card[]): HandResult {
  ...
  /** Group cards by suit */
  _groupBySuit(cards: Card[]): Record<string, Card[]> {
  ...
  _groupByValue(cards: Card[]): Record<number, Card[]> {
  ...
  _highestCards(cards: Card[], n: number): Card[] {
    return [...cards].sort((a, b) => b.value - a.value).slice(0, n);
  },

  /**
   * Find the best straight in a set of cards.
   * @returns { highValue, cards: Card[5] } or null
   */
  _findStraightInCards(cards: Card[]): StraightResult | null {
    ...
    if (hasAce && has2 && has3 && has4 && has5) {
      ...
      const getCard = (val: number, preferSuit?: string): Card | undefined => {
        if (val === 14) {
      ...
      if (wheelCards.every((c): c is Card => c !== undefined)) {
...
if (require.main === module) {
  function C(suit: string, rank: string) { return new Card(suit, rank); }

  console.log('=== HandEvaluator Self-Test ===\n');

  const tests: Array<{ name: string; cards: Card[]; expectRank: number; expectKicker?: number }> = [
    {
      name: 'Royal Flush',
      cards: [
