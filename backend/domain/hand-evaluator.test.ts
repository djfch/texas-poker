/**
 * backend/domain/hand-evaluator.test.ts
 *
 * Comprehensive unit tests for the HandEvaluator.
 * Run with: node --import tsx --test backend/domain/hand-evaluator.test.ts
 *
...
function assertHandResult(result: HandResult, expected: ExpectedHand): void {
...
describe('HandEvaluator', () => {
  ...
  test('Performance - 1000 evaluations under 500ms', () => {
    ...
    // Generous bound: catches order-of-magnitude regressions only. The old
    // 100ms limit flaked under CI/host load (typically ~30-120ms observed).
    assert.ok(elapsed < 500, `1000 evaluations took ${elapsed.toFixed(2)}ms, expected < 500ms`);
  });
  ...
  test('Rejects non-array input', () => {
    // @ts-expect-error - intentionally invalid input to verify runtime validation
