/**
 * backend/domain/pot-manager.ts - Pot Calculation Engine
 *
...
/** Serializable PotManager snapshot. */
export interface PotManagerJSON {
  bets: BetInfo[];
}
...
export interface WinnerEntry {
  position: number;
  handResult: HandResultLike;
}

export type CompareFn = (a: WinnerEntry, b: WinnerEntry) => number;

export class PotManager {
  /** position -> bet info */
  private bets: Map<number, BetInfo>;

  /**
   * @param seats - Array of seat objects:
   *   { position, totalBet, status: 'folded'|'active'|'allin', chips }
   */
  constructor(seats: SeatInput[] = []) {
    this.bets = new Map();
   ...
   * @param amount - Amount to add to totalBet
   */
  addBet(position: number, amount: number): void {
   ...
   */
  getBet(position: number): BetInfo | null {
   ...
   *
   * @returns { mainPot: number, sidePots: Array<{amount, eligiblePositions}> }
   */
  calculatePots(): PotBreakdown {
   ...
   *
   * @param winners - Array of { position, handResult } for each non-folded player
   *   handResult should be comparable (has .rank and .kickers, or use HandEvaluator.compare)
   * @param compareFn - Optional comparison function(handA, handB) -> -1|0|1
   *   Defaults to comparing handResult objects directly (must have rank+kickers).
   * @returns Map of position -> payout amount
   */
  distribute(winners: WinnerEntry[], compareFn: CompareFn | null = null): Record<number, number> {
   ...
   * @param status - 'active', 'folded', 'allin'
   */
  setStatus(position: number, status: SeatStatus): void {
    if (!['active', 'folded', 'allin'].includes(status)) {
  ...
  /**
   * @returns Serializable snapshot: one plain bet entry per seat, in
   * insertion order. JSON.stringify picks this up automatically, so a
   * PotManager nested inside a larger record survives a round-trip.
   */
  toJSON(): PotManagerJSON {
    return { bets: Array.from(this.bets.values()).map(info => ({ ...info })) };
  }

  /**
   * Restore a PotManager from a plain snapshot (e.g. after a Redis
   * round-trip). Routes through the constructor so the usual seat
   * validation (finite position, non-negative totalBet, known status)
   * applies to the restored data as well.
   */
  static fromJSON(obj: any): PotManager {
    if (!obj || !Array.isArray(obj.bets)) {
      throw new Error('Invalid pot-manager JSON: expected an object with a bets array');
    }
    return new PotManager(obj.bets);
  }

  // ─── Internal ──────────────────────────────────────────────────

  /** Calculate all pots with eligible positions (including main pot). */
  private _calculateAllPots(): SidePot[] {
