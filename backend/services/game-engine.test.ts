...
async function createPlayer(id: string, seatPosition: number = id === 'human-1' ? 0 : 1) {
  const player = {
    id,
    nickname: id,
    avatar: '#2ecc71',
...
test('private deal payloads include two serializable hole cards for each seated player', async () => {
  ...
  assert.deepEqual(deals.map((d: any) => d.position).sort(), [0, 1]);
...
test('AI decision context includes legal actions, action history, position and odds', async () => {
  ...
  assert.deepEqual(context.action_history.preflop.map((a: any) => a.action), ['small_blind', 'big_blind']);
  assert.deepEqual(context.action_history.preflop.map((a: any) => a.amount), [10, 20]);
  assert.deepEqual(context.action_history.preflop.map((a: any) => a.pot_after), [10, 30]);
});
...
test('uncontested showdown does not evaluate a winner with missing hole cards', async () => {
  ...
  game.players.find((p: any) => p.playerId === 'human-2').holeCards = [];
...
test('all-in reveals every non-folded player hole cards only when all active players are all-in', async () => {
  ...
  assert.equal(beforeAllIn.players.every((p: any) => p.holeCards === null), true);
  ...
  assert.equal(afterFirstAllIn.players.every((p: any) => p.holeCards === null), true);
  ...
  assert.equal(afterSecondAllIn.players.every((p: any) => p.holeCards === null), true);
  ...
  const livePlayers = afterAllAllIn.players.filter((p: any) => !p.folded);
  assert.equal(livePlayers.length, 3);
  assert.equal(livePlayers.every((p: any) => Array.isArray(p.holeCards) && p.holeCards.length === 2), true);
});
...
test('all-in action result includes revealed live player hole cards only after all active players are all-in', async () => {
  ...
  assert.equal(
    firstResult.game.players.filter((p: any) => !p.folded).every((p: any) => p.holeCards === null),
    true
  );
  ...
  assert.equal(
    finalResult.game.players.filter((p: any) => !p.folded).every((p: any) => Array.isArray(p.holeCards) && p.holeCards.length === 2),
    true
  );
});
...
test('ended hands include every player hand result delta from the start of the hand', async () => {
  ...
  assert.equal(result.success, true);
  assert.deepEqual(result.game.handResults.map((r: any) => ({
    playerId: r.playerId,
    position: r.position,
    nickname: r.nickname,
    chips: r.chips,
...
test('ended hands reset human readiness while AI players auto-ready for the next hand', async () => {
  ...
  assert.equal(room.players.find((p: any) => p.playerId === 'human-1').isReady, false);
  assert.equal(room.players.find((p: any) => p.playerId === 'bot-1').isReady, true);
...
test('nextHand leaves broke seated players at zero chips until they borrow', async () => {
  ...
  assert.equal(result.success, true);
  assert.equal(result.room.players.find((p: any) => p.playerId === 'human-1').chips, 0);
...
test('keeps action on the last player who still faces multiple all-ins', async () => {
  ...
  assert.deepEqual(
    (await gameEngine.getValidActions('ROOM01', 'human-3')).map((action: any) => action.type),
    ['fold', 'allin']
  );
});

test('invalid action is rejected and the turn does not advance', async () => {
  resetStore();
  await createPlayer('human-1', 0);
  await createPlayer('human-2', 1);
  await createPlayer('human-3', 2);
