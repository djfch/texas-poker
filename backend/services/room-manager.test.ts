...
test('host leaving settles everyone and deletes the room', async () => {
  ...
  assert.deepEqual(result.settlements.map((s: any) => ({
    playerId: s.playerId,
    chips: s.chips,
...
test('joining a room uses room initial chips and initializes buy-in ledger', async () => {
  ...
  const joined = room.players.find((p: any) => p.playerId === 'new-human');
...
test('leaving as a non-host settles only that player and clears their room chips', async () => {
  ...
  assert.equal(room.players.some((p: any) => p.playerId === 'human-2'), false);
...
test('adds exactly one AI player to the first open seat', async () => {
  ...
  const fakeAIManager = {
    createdPositions: [] as number[],
    async createBot(roomId: string, position: number) {
...
test('auto-lend gives broke seated AI one initial stack and marks them ready', async () => {
  ...
  const room = await store.getRoom('ROOM01');
  const bot = room.players.find((p: any) => p.playerId === 'bot-1');
...
test('removes an AI player from a requested waiting-room seat', async () => {
  ...
  const fakeAIManager = {
    removedPositions: [] as number[],
    async removeBot(roomId: string, position: number) {
  ...
  assert.equal(room.players.some((p: any) => p.playerId === 'bot-1'), false);
});

test('an in-room member can re-join a playing room (idempotent reconnect)', async () => {
  resetStore();
  await createPlayer('old-host', false);
  await store.createRoom(createRoom({ status: 'playing' }));

  const result = await roomManager.joinRoom('ROOM01', 'old-host');

  assert.equal(result.success, true);
  assert.equal(result.room.id, 'ROOM01');
  assert.equal(result.room.status, 'playing');
});

test('an outsider is still rejected from a playing room', async () => {
  resetStore();
  await createPlayer('old-host', false);
  await createPlayer('stranger', false);
  await store.createRoom(createRoom({ status: 'playing' }));

  const result = await roomManager.joinRoom('ROOM01', 'stranger');

  assert.equal(result.success, false);
  assert.equal(result.error, 'Game in progress');
  assert.ok(!(await store.getRoom('ROOM01')).players.some((p: any) => p.playerId === 'stranger'));
});
